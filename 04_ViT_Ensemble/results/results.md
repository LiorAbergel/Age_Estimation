...
> **Role in paper:** Table 3 (middle section) — ViT ensembles on the official
> HHD split (single-split results on the official HHD test set).
...
## Hybrid CNN+ViT Ensemble (Table 4)

The paper's hybrid ensemble (Table 4) is a simple mean of the best individual CNN
(InceptionV3, from `01_CNN_Ensemble`) and the best individual ViT (MobileViT-XXS),
evaluated on the official HHD test split. It improves RMSE, R², and median error
over either constituent, but does not lower MAE below MobileViT-XXS alone —
indicating only partial complementarity between the two feature-extraction paradigms.

| Metric | InceptionV3 | MobileViT-XXS | Ensemble (mean) |
|--------|------------:|--------------:|----------------:|
| MAE | 3.31 | **2.79** | 2.81 |
| RMSE | 6.20 | 6.13 | **6.09** |
| R² | 0.04 | 0.06 | **0.07** |
| MAPE (%) | 18.05 | **13.59** | 14.33 |
| ±2 yrs (%) | 48.28 | **65.52** | 62.93 |
| ±5 yrs (%) | 87.07 | **93.97** | 90.52 |
| Max Error | **33.98** | 34.91 | 34.44 |
| Min Error | **0.06** | 0.05 | 0.02 |
| Median Error | 2.04 | 1.35 | **1.29** |

Reproduce this table from the committed predictions (no GPU, no weights):
